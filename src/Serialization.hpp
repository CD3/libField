...
#error "Boost member template friends must be disabled in order to serialize mulati_array."
#endif
...
inline void save( Archive &ar, const boost::multi_array<T,N>& a, const unsigned int version)
{
...
template<class Archive,typename T,std::size_t N>
inline void load( Archive &ar, boost::multi_array<T,N>& a, const unsigned int version)
{
  ...
  for(int i = 0; i < N; i++)
    a.stride_list_[i] = strides[i];

}

template<typename Archive,typename T,std::size_t N>
inline void serialize(Archive & ar, boost::multi_array<T,N>& a, const unsigned int version)
{
