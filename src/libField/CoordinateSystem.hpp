...
class CoordinateSystem
{
 ...
 public:
  ...
  /** For a 1D coordinate system, return the coordinate directly, instead of an
 ...
 protected:
  ...
  void nearest_imp(IND& ind, C c, Args... args) const
  {
    ...
    // coordinate is greater than largest
      ...
      // coord - lower bound divided by upper bound minus lower bound gives the
      // dimensionless coordinate between 0 and 1. if this is less than 0.5, we
      // want to return the lower bound index. if itis greater than 0.5, we want
      // to return the upper bound index don't have to worry about special cases
      // above here.
      ind[II] +=
          2 * (c - axes[II]->operator[](ind[II])) /
          (axes[II]->operator[](ind[II] + 1) - axes[II]->operator[](ind[II]));
    }
