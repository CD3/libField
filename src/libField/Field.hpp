 ...
 * @date 06/13/17
 *
 * A field is a quantity defined at every point in space. For example, the
 * temperature distribution throughout a solid, the pressure at every point in a
 * room, or the density of a fluid. A field may be 1-, 2-, or 3-dimensional, and
 * is parameterized by a coordinate system.
 *
 * This class associates a coordinate system with a multi-dimensional array.
 * Field elements are allocated in a single, multi-dimensional array. And a
 * CoordinateSystem is allocated for the coordinates.
 */
...
class Field {
   ...
   public:
     ...
     *
     * Memory can be allocated later with Field::reset() method. The default
     * constructor is provided to support storing fields in some containers that
     ...
     *
     * This constructor allows the caller to pass dimension sizes directly to
     * the constructor.
     *
     * @code
     ...
     * @endcode
     *
     * This will create a field with 200 elements, 10 along the first dimension,
     * 20 along the second.
     *
     */
     ...
     *
     * @param sizes an array of integers specifying the size of the field along
     * each dimension.
     */
    template <typename I>
    ...
    /**
     * @brief Reallocate a field from an existing coordinate system and field
     * elements.
     *
     * @param cs_ a shared pointer to an existing coordinate system.
     ...
     * Example:
     *
     * The elements of a 2-dimensional array can be accessed using a 2-element
     * vector.
     * @code
     ...
     * @endcode
     *
     * Any container that provides a subscript operator (operator[](int)) can be
     * used.
     */
    template <typename I,
     ...
     *
     * This function is a variadic template that allow a natural access to the
     * field elements using the operator()(), rather than having to use an index
     * container..
     *
     * Example:
     *
    ...
    /**
     * @brief Return a shared pointer to the coordinate system used by the
     * field.
     */
    ...
    /**
     * @brief Return a reference to the i'th axis in the coordinate system used
     * by the field.
    ...
    /**
     * @brief Return a const reference to the coordinate system used by the
     * field.
     */
    const auto& getCoordinateSystem() const { return *cs; };
    /**
     * @brief Return a const reference to the i'th axis in the coordinate system
     * used by the field.
     * @param i The index (zero-offset) of the axis to return.
     */
     ...
     * @param args Coordinates specification. The arguments are passed directly
     * to CoordinateSystem::set() of the coordinate system used by the field.
     *
     * Coordinate system coordinate are set using a range discretizer. See
     * range_discretizers::UnitformImp<T> for example.
     *
     * Example:
     *
     * To configure a field over the range [-1,1] along the x direction and
     * [0:4] along the y direction,
     * @code
     * Field<double,2> f(10,20);
     ...
     * @param args The index of the coordinate to retrief. The arguments are
     * passed directly to CoordinateSystem::getCoord() of the coordinate system
     * used by the field.
     *
     * Example:
     *
     ...
     * ...
     *
     * auto coord = f.getCoord(2,4); // get the coordinate for the field element
     * at (2,4) double x = coord[0]; // get the x coordinate; double y =
     * coord[1]; // get the x coordinate;
     * @endcode
     */
    template <typename... Args>
    ...
    /**
     * @brief Returns index of stored coordinate that lower bounds the given
     * coordinate.
     *
     * This function forwards the arguments to Coordinate::lower_bound() of the
     * coordinate system used by the field. Along with upper_bound, it is useful
     * for finding a range that bounds the given coordinate.
     */
    template <typename... Args>
    ...
    /**
     * @brief Returns index of stored coordinate that upper bounds the given
     * coordinate.
     *
     * This function forwards the arguments to Coordinate::upper_bound() of the
     * coordinate system used by the field. Along with lower_bound, it is useful
     * for finding a range that bounds the given coordinate.
     */
    template <typename... Args>
    ...
    /**
     * @brief returns index of stored coordinate that is closest to the given
     * coordinate.
     */
    template <typename... Args>
    ...
    template <int NDims>
    const auto slice(
        const boost::detail::multi_array::index_gen<NUMDIMS, NDims>& ind)
        const {
        // get sliced data
    ...
    /**
     * @brief Set each element of a field using a callable that takes an
     * array-like container of *coordinates* as an argument and returns the
     * element's value.
     *
     * This function evaluates the callable f for each coordinate and sets the
     * field value to value returned by callable. Field may be evaluated in
     * PARRALLEL. Callable should NOT depend on the order of being called.
     *
     * @param f a callable object (function, funtor, lambda, std::function,
     * etc.) that accepts one argument and returns a value.
     *
...
#pragma omp parallel for
     ...
     * element's value.
     *
     * This function evaluates the callable f that returns an optional type
     * (boost::optional or std::optional) for each coordinate. If the optional
     * is set, then the value of of the field element is set. Otherwise, the
     * field is left untouched. Field may be evaluated in PARRALLEL. Callable
     * should NOT depend on the order of being called.
     *
     * @param f a callable object (function, funtor, lambda, std::function,
     * etc.) that accepts one argument and returns an optional value.
     *
     * Argument passed to callable f will be an array of coordinates.
     *
     */
...
#pragma omp parallel for
    ...
    /**
     * @brief Set each element of a field using a callable that takes an
     * array-like container of *indices* and a pointer to a coordinate system as
     * arguments and returns the element's value.
     *
     ...
     *
     * @param f a callable object (function, funtor, lambda, std::function,
     * etc.) that accepts two arguments and returns a value.
     *
...
#pragma omp parallel for
     ...
     * arguments and returns the element's value.
     *
     * Evaluates callable f for each coordinate index. If the optional returned
     * by f is set, the field for the coordinate is set. Otherwise, the field is
     * left untouched. Callable is passed a container of indices and a pointer
     * to the coordinate system. Field may be evaluated in PARRALLEL. Callable
     * should NOT depend on the order of being called.
     *
     * @param f a callable object (function, funtor, lambda, std::function,
     * etc.) that accepts two arguments and returns a value.
...
#pragma omp parallel for
    ...
    // NOTE: we wanted to combined set and set_f into a single function, but
    // this isn't possible in general. We cannot assume that the set_f version
    // should be called if a function is passed in, because the user may
...
#pragma omp parallel for
    ...
    /**
     * @brief Subtract the element of a second field from each element of the
     * field.
...
#pragma omp parallel for
    ...
    /**
     * @brief Multiply each element in the field by the corresponding element in
     * a second field.
...
#pragma omp parallel for
    ...
    /**
     * @brief Divide each element in the field by the corresponding element in a
     * second field.
