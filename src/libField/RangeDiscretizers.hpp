...
class GeometricWithPeriodImp {
   public:
    GeometricWithPeriodImp(T min, T dx, T stretch, T period) {
        ...
        this->stretch = stretch;
        this->period = period;
        this->Np = 1 + log(1 - (this->period / this->dx) * (1 - stretch)) /
                           log(stretch);  // the number of points that can fit
                                          // inside one period
        assert(stretch > 1);
    }
...
// using a factory function here so that argument types can be deduced.
template <typename T, typename S>
GeometricWithPeriodImp<T> GeometricWithPeriod(T min, T dx, S stretch,
                                              T period) {
