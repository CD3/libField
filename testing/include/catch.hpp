...
#define INTERNAL_CATCH_REMOVE_PARENS_7_ARG(_0, _1, _2, _3, _4, _5, _6) \
  INTERNAL_CATCH_REMOVE_PARENS(_0),                                    \
      INTERNAL_CATCH_REMOVE_PARENS_6_ARG(_1, _2, _4, _5, _6)
...
#define INTERNAL_CATCH_TYPE_GEN                                              \
  ...
  constexpr auto append(L1<E1...>, L2<E2...>, Rest...) noexcept->decltype(   \
      append(L1<E1..., E2...>{}, Rest{}...))                                 \
  {                                                                          \
    return {};                                                               \
  }                                                                          \
  template<template<typename...> class L1, typename... E1, typename... Rest> \
  constexpr auto append(L1<E1...>, TypeList<mpl_::na>,                       \
                        Rest...) noexcept->L1<E1...>                         \
  {                                                                          \
    return {};                                                               \
  }                                                                          \
  ...
  constexpr auto rewrap(                                                     \
      List<elems...>) noexcept->TypeList<Container<elems...>>                \
  {                                                                          \
    return {};                                                               \
  }                                                                          \
  template<template<typename...> class Container,                            \
  ...
  constexpr auto rewrap(List<Elems...>, Elements...) noexcept->decltype(     \
      append(TypeList<Container<Elems...>>{},                                \
             rewrap<Container>(Elements{}...)))                              \
  {                                                                          \
    return {};                                                               \
  }                                                                          \
                                                                             \
  ...
  constexpr auto convert(List<Ts...>) noexcept->decltype(                    \
      append(Final<>{}, TypeList<Ts>{}...))                                  \
  {                                                                          \
    return {};                                                               \
  }
...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                 \
  ...
  constexpr auto rewrap(                                                      \
      List<__VA_ARGS__>) noexcept->TypeList<Container<__VA_ARGS__>>           \
  {                                                                           \
    return {};                                                                \
  }                                                                           \
  template<template<INTERNAL_CATCH_REMOVE_PARENS(signature)> class Container, \
  ...
  constexpr auto rewrap(List<__VA_ARGS__>, Elements... elems) noexcept        \
      ->decltype(append(TypeList<Container<__VA_ARGS__>>{},                   \
                        rewrap<Container>(elems...)))                         \
  {                                                                           \
    return {};                                                                \
  }                                                                           \
  template<template<typename...> class Final,                                 \
...
#define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE_METHOD_2(                    \
  ...
  {                                                                            \
    ...
    static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {            \
      using TestInit = decltype(                                               \
          create<TestNameClass, INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>(      \
              TypeList<INTERNAL_CATCH_MAKE_TYPE_LISTS_FROM_TYPES(              \
                  INTERNAL_CATCH_REMOVE_PARENS(TypesList))>{}));               \
      TestInit t;                                                              \
      t.reg_tests();                                                           \
...
struct StringHolder : MatcherBase<NSString*> {
  ...
  bool match(NSString* const& str) const override { return false; }
...
template<typename Fun, typename... Args>
CompleteType_t<ResultOf_t<Fun(Args...)>> complete_invoke(Fun&& fun,
                                                         Args&&... args)
{
  return CompleteInvoker<ResultOf_t<Fun(Args...)>>::invoke(
      std::forward<Fun>(fun), std::forward<Args>(args)...);
}
...
using TimingOf = Timing<ClockDuration<Clock>,
                        Detail::CompleteType_t<Detail::ResultOf_t<Sig>>>;
}  // namespace Benchmark
}  // namespace Catch
...
struct Benchmark {
      ...
      const IConfig& cfg, Environment<FloatDuration<Clock>> env) const
  {
    ...
    auto run_time = std::max(
        min_time,
        std::chrono::duration_cast<decltype(min_time)>(Detail::warmup_time));
...
std::string WithinUlpsMatcher::describe() const
{
  ...
  // ::Catch::Detail::stringify(m_target) + ((m_type ==
  // FloatingPointKind::Float)? "f" : "");
}
...
Catch::Totals runTests(std::shared_ptr<Config> const& config)
{
  ...
  for (auto const& testCase : allTestCases) {
    bool matching =
        (!testSpec.hasFilters() && !testCase.isHidden()) ||
...
bool SectionTracker::isComplete() const
{
  ...
  if ((m_filters.empty() || m_filters[0] == "") ||
      std::find(m_filters.begin(), m_filters.end(), m_nameAndLocation.name) !=
          m_filters.end())
...
ConsoleReporter::ConsoleReporter(ReporterConfig const& config)
    : StreamingReporterBase(config),
      m_tablePrinter(new TablePrinter(
          config.stream(),
          {{"benchmark name", CATCH_CONFIG_CONSOLE_WIDTH - 32,
            ColumnInfo::Left},
...
void ConsoleReporter::benchmarkStarting(BenchmarkInfo const& info)
{
  (*m_tablePrinter) << info.samples << ColumnBreak() << info.iterations
                    << ColumnBreak() << Duration(info.estimatedDuration)
                    << ColumnBreak();
}
...
void XmlReporter::benchmarkStarting(BenchmarkInfo const& info)
{
  m_xml.startElement("BenchmarkResults")
      ...
      .writeAttribute("clockResolution",
                      static_cast<uint64_t>(info.clockResolution))
      .writeAttribute("estimatedDuration",
                      static_cast<uint64_t>(info.estimatedDuration))
...
void XmlReporter::benchmarkEnded(BenchmarkStats<> const& benchmarkStats)
{
  m_xml.startElement("mean")
      .writeAttribute("value",
                      static_cast<uint64_t>(benchmarkStats.mean.point.count()))
      .writeAttribute(
          "lowerBound",
